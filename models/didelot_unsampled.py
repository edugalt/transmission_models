...
class didelot_unsampled():
    ...
    def __init__(self, sampling_params, offspring_params, infection_params):
        ...
        self.N_candidates_to_offspring = 0
        self.candidates_to_offspring = []
    ...
    def get_candidates_to_offspring(self):
        self.candidates_to_offspring = [h for h in self.T if not h==self.root_host and not self.out_degree(self.parent(h))==1]
        return self.candidates_to_offspring

    def get_N_candidates_to_offspring(self,recompute=False):
        if recompute: return len(self.get_candidates_to_offspring())
        else: len(self.candidates_to_offspring)
    ...
    def infection_time_from_sampling_step(self, verbose=False):
        """
